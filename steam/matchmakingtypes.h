...
// stdlib.h added for exit(int)
// Not clear why valve is using the C headers over the C++ equivalents -
// this is C++ code, not C.
#include <stdlib.h>

// stdio+string already existed.
#include <stdio.h>
#include <string.h>
...
inline const char *servernetadr_t::ToString( uint32 unIP, uint16 usPort ) const
{
  ...
  // TODO: These appear to be platform independant snprintf functions
  //       that are defined by Valve in tier1/strtools.h  There's appears 
  //       to be some other magic going on as well.  As the Q_* is only 
  //       lightly used in a few places that aren't actually implemented 
  //       by SteamRedirect yet, just going to comment it all out for now.
  //       It's entirely possibly (and perhaps likely) that a "#define
  //       Q_snprintf snprintf" works fine for the single target platform 
  //       of SteamRedirect.
  //
  // Exit on the off chance we get to here before fixing this, justincase.
  (void)(usPort); // down with meaningless compiler warnings
  (void)(ipByte);
  printf("ERROR: const char *servernetadr_t::ToString(uint32,uint16) const:\n"
      "this function is intentionally broken as it's not likely to be used in the\n"
      "near future, and creates compilation issues that aren't worth fixing at the\n"
      "moment.  Quiting to avoid any problems...\n");
  exit(2);
